...
class MoodleQuickForm_wikieditor extends MoodleQuickForm_textarea {
    ...
    private function getButtons() {
        ...
        foreach ($wiki_editor as $button) {
            ...
            $html .= "<img src=\"$CFG->wwwroot/mod/wiki/editors/wiki/images/$button[0]\" alt=\"".$button[1]."\" title=\"".$button[1]."\" />";
            $html .= "</a>";
        }
        $html .= "<select onchange=\"insertTags('{$imagetag[0]}', '{$imagetag[1]}', this.value)\">";
        $html .= "<option value='" . get_string('wikiimage', 'wiki') . "'>" . get_string('insertimage', 'wiki') . '</option>';
