...
class course_modinfo {
    ...
    public function __construct($course, $userid) {
        ...
        if ($coursemodinfo === false || ($course->cacherev != $coursemodinfo->cacherev)) {
            ...
            try {
                ...
                $coursemodinfo = $cachecoursemodinfo->get($course->id);
                if ($coursemodinfo === false || ($course->cacherev != $coursemodinfo->cacherev)) {
                    $coursemodinfo = self::inner_build_course_cache($course, $lock, false);
                }
            } finally {
                $lock->release();
            }
     ...
     */
    protected static function build_course_section_cache($course, $usecache = false) {
        global $DB;
     ...
     * @param stdClass $partialrebuild only rebuild when ncecssary
     ...
     */
    protected static function inner_build_course_cache($course, \core\lock\lock $lock, $partialrebuild=false) {
        global $DB, $CFG;
