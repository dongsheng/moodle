            ...
            '<form class="atto_form">' +
                ...
                '<div class="mb-1">' +
                    ...
                    '<div id="{{elementid}}_{{CSS.INPUTSIZE}}" class="form-inline {{CSS.INPUTSIZE}}">' +
                        '<label class="sr-only" for="{{elementid}}_{{CSS.INPUTWIDTH}}">' +
                            '{{get_string "width" component}}</label>' +
                        '<input type="text" class="form-control mr-1 input-mini {{CSS.INPUTWIDTH}}" ' +
                        'id="{{elementid}}_{{CSS.INPUTWIDTH}}" size="4"/>' +
                            ' x ' +
                        // Add the height entry box.
                        '<label class="sr-only" for="{{elementid}}_{{CSS.INPUTHEIGHT}}">' +
                            '{{get_string "height" component}}</label>' +
                ...
                '<label for="{{elementid}}_{{CSS.INPUTALIGNMENT}}">{{get_string "alignment" component}}</label>' +
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _loadPreviewImage: function(url) {
        ...
        image.onerror = function() {
            var preview = self._form.one(SELECTORS.IMAGEPREVIEW);
            preview.setStyles({
                'display': 'none'
            });
        ...
        image.onload = function() {
            ...
            input = self._form.one(SELECTORS.INPUTWIDTH);
            currentwidth = input.get('value');
            if (currentwidth === '') {
                input.set('value', this.width);
                currentwidth = "" + this.width;
            }
            input = self._form.one(SELECTORS.INPUTHEIGHT);
            currentheight = input.get('value');
            if (currentheight === '') {
                input.set('value', this.height);
                currentheight = "" + this.height;
            }
            input = self._form.one(SELECTORS.IMAGEPREVIEW);
            input.setAttribute('src', this.src);
            input.setStyles({
                'display': 'inline'
            });

            input = self._form.one(SELECTORS.INPUTCONSTRAIN);
    ...
    _getDialogueContent: function() {
        ...
        this._form.one(SELECTORS.IMAGEPRESENTATION).on('change', this._updateAltWarning, this);
        this._form.one(SELECTORS.INPUTALT).on('blur', this._updateAltWarning, this);
        this._form.one(SELECTORS.INPUTWIDTH).on('blur', this._autoAdjustSize, this);
        this._form.one(SELECTORS.INPUTHEIGHT).on('blur', this._autoAdjustSize, this, true);
        this._form.one(SELECTORS.INPUTCONSTRAIN).on('change', function(event) {
            if (event.target.get('checked')) {
                this._autoAdjustSize(event);
            }
        }, this);
        this._form.one(SELECTORS.INPUTSUBMIT).on('click', this._setImage, this);

        if (canShowFilepicker) {
            this._form.one(SELECTORS.IMAGEBROWSER).on('click', function() {
                    this.get('host').showFilepicker('image', this._filepickerCallback, this);
            }, this);
        }

        // Character count.
        this._form.one(SELECTORS.INPUTALT).on('keyup', this._handleKeyup, this);
    ...
    _autoAdjustSize: function(e, forceHeight) {
        ...
        var keyField = this._form.one(SELECTORS.INPUTWIDTH),
            keyFieldType = 'width',
            subField = this._form.one(SELECTORS.INPUTHEIGHT),
            subFieldType = 'height',
            constrainField = this._form.one(SELECTORS.INPUTCONSTRAIN),
            keyFieldValue = keyField.get('value'),
            subFieldValue = subField.get('value'),
            imagePreview = this._form.one(SELECTORS.IMAGEPREVIEW),
            rawPercentage,
            rawSize;
    ...
    _filepickerCallback: function(params) {
        if (params.url !== '') {
            ...
            this._form.one(SELECTORS.INPUTWIDTH).set('value', '');
            this._form.one(SELECTORS.INPUTHEIGHT).set('value', '');

            this._updateUrlWarning();
    ...
    _applyImageProperties: function(form) {
        var properties = this._getSelectedImageProperties(),
            img = form.one(SELECTORS.IMAGEPREVIEW);
        ...
        if (properties === false) {
            ...
            ALIGNMENTS.some(function(alignment) {
                if (alignment.isDefault) {
                    form.one(SELECTORS.INPUTALIGNMENT).set('value', alignment.value);
                    return true;
                }
        ...
        if (properties.align) {
            form.one(SELECTORS.INPUTALIGNMENT).set('value', properties.align);
        }
        if (properties.customstyle) {
            form.one(SELECTORS.INPUTCUSTOMSTYLE).set('value', properties.customstyle);
        }
        if (properties.width) {
            form.one(SELECTORS.INPUTWIDTH).set('value', properties.width);
        }
        if (properties.height) {
            form.one(SELECTORS.INPUTHEIGHT).set('value', properties.height);
        }
        if (properties.alt) {
            form.one(SELECTORS.INPUTALT).set('value', properties.alt);
        }
        if (properties.src) {
        ...
        if (properties.presentation) {
            form.one(SELECTORS.IMAGEPRESENTATION).set('checked', 'checked');
        }
    ...
    _setImage: function(e) {
        var form = this._form,
            ...
            alt = form.one(SELECTORS.INPUTALT).get('value'),
            width = form.one(SELECTORS.INPUTWIDTH).get('value'),
            height = form.one(SELECTORS.INPUTHEIGHT).get('value'),
            alignment = this._getAlignmentClass(form.one(SELECTORS.INPUTALIGNMENT).get('value')),
            presentation = form.one(SELECTORS.IMAGEPRESENTATION).get('checked'),
            constrain = form.one(SELECTORS.INPUTCONSTRAIN).get('checked'),
            imagehtml,
            customstyle = form.one(SELECTORS.INPUTCUSTOMSTYLE).get('value'),
            classlist = [],
            host = this.get('host');
        ...
        var urlerror = this._updateUrlWarning();
        var alterror = this._updateAltWarning();
        if (urlerror || alterror) {
            return;
        }
        ...
        if (url !== '') {
            ...
            if (!width.match(REGEX.ISPERCENT) && isNaN(parseInt(width, 10))) {
                form.one(SELECTORS.INPUTWIDTH).focus();
                return;
            }
            if (!height.match(REGEX.ISPERCENT) && isNaN(parseInt(height, 10))) {
                form.one(SELECTORS.INPUTHEIGHT).focus();
                return;
            }
    ...
    /**
     * Update the url text warning live.
     *
     * @method _updateUrlWarning
     * @return {boolean} whether a warning should be displayed.
     * @private
     */
    _updateUrlWarning: function() {
        var form = this._form,
            url = form.one(SELECTORS.INPUTURL).get('value');
        var urlerror = url === '';
        this._toggleVisibility(SELECTORS.IMAGEURLWARNING, urlerror);
        this._toggleAriaInvalid([SELECTORS.INPUTURL], urlerror);
    ...
    /**
     * Update the image alt text warning.
     *
     * @method _updateAltWarning
     * @return {boolean} whether a warning should be displayed.
     * @private
     */
    _updateAltWarning: function() {
        var form = this._form,
            alt = form.one(SELECTORS.INPUTALT).get('value'),
            presentation = form.one(SELECTORS.IMAGEPRESENTATION).get('checked');
        var imagealterror = alt === '' && !presentation;
        this._toggleVisibility(SELECTORS.IMAGEALTWARNING, imagealterror);
        this._toggleAriaInvalid([SELECTORS.INPUTALT, SELECTORS.IMAGEPRESENTATION], imagealterror);
