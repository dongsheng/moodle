...
var COMPONENTNAME = 'atto_media',
    ...
    CSS = {
        ...
        MEDIA_URL_WARNING: 'atto_media_mediaurlwarning',
    ...
    TEMPLATES = {
        ...
        TAB_PANES: {
            LINK: '' +
                '{{renderPartial "form_components.source" parenttab="link" context=this id=CSS.LINK_SOURCE urlinput="link"}}' +
            ...
            VIDEO: '' +
                '{{renderPartial "form_components.source" parenttab="video" context=this id=CSS.MEDIA_SOURCE ' +
                    ' entersourcelabel="videosourcelabel"' +
                    ' addcomponentlabel="addsource" multisource="true" addsourcehelp=helpStrings.addsource urlinput="video"}}' +
                '<fieldset class="collapsible collapsed" id="{{elementid}}_video-display-options">' +
                    ...
                    '<div class="fcontainer">' +
                        '{{renderPartial "form_components.display_options" parenttab="video" ' +
                        ' context=this id=CSS.VIDEO mediatype_video=true}}' +
                    '</div>' +
                '</fieldset>' +
                ...
                '</fieldset>',
            AUDIO: '' +
                '{{renderPartial "form_components.source" parenttab="audio" context=this id=CSS.MEDIA_SOURCE ' +
                    ' entersourcelabel="audiosourcelabel" addcomponentlabel="addsource" multisource="true" ' +
                    ' addsourcehelp=helpStrings.addsource urlinput="audio"}}' +
                '<fieldset class="collapsible collapsed" id="{{elementid}}_audio-display-options">' +
                    ...
                    '<legend class="ftoggler">{{get_string "displayoptions" component}}</legend>' +
                    '<div class="fcontainer">' +
                        '{{renderPartial "form_components.display_options" parenttab="audio" context=this id=CSS.AUDIO}}' +
                    '</div>' +
                '</fieldset>' +
        ...
        FORM_COMPONENTS: {
            SOURCE: '' +
                '<div class="{{CSS.SOURCE}} {{id}}">' +
                    '<div class="mb-1">' +
                        '<label for="{{concat parenttab urlinput}}-url-input-{{suffix}}">' +
                          ...
                          '<div style="display:none" role="alert" class="alert alert-warning mb-1 {{CSS.MEDIA_URL_WARNING}}">' +
                              '{{get_string "mediaurlrequired" component}}' +
                          '</div>' +
                        '</label>' +
                        '<div class="input-group input-append w-100">' +
                            '<input id="{{concat parenttab urlinput}}-url-input-{{suffix}}" ' +
                                ' class="form-control {{CSS.URL_INPUT}}" type="url" size="32"/>' +
                ...
                '<div class="{{CSS.DISPLAY_OPTIONS}}">' +
                    ...
                    '<div class="mb-1">' +
                        ...
                        '<div class="form-inline {{CSS.POSTER_SIZE}}">' +
                            '<label for="{{parenttab}}-width-input" class="sr-only">{{get_string "videowidth" component}}</label>' +
                            '<input id="{{parenttab}}-width-input" type="text" size="4" ' +
                              ' class="form-control mr-1 {{CSS.WIDTH_INPUT}} input-mini" />' +
                              ' x ' +
                            '<label for="{{parenttab}}-height-input" class="sr-only">' +
                              '{{get_string "videoheight" component}}' +
                            '</label>' +
                            '<input id="{{parenttab}}-height-input" type="text" ' +
                              ' class="form-control ml-1 {{CSS.HEIGHT_INPUT}} input-mini" size="4"/>' +
                        '</div>' +
                    '</div>' +
                    '<div class="clearfix"></div>' +
                    '{{renderPartial "form_components.source" parenttab=parenttab context=this id=CSS.POSTER_SOURCE ' +
                        ' entersourcelabel="poster" urlinput="poster"}}' +
                    '{{/mediatype_video}}' +
                '<div>',
            ADVANCED_SETTINGS: '' +
                ...
                '<div class="tab-content">' +
                    '<div data-track-kind="{{CSS.TRACK_SUBTITLES}}" class="tab-pane active"' +
                        ...
                        '{{renderPartial "form_components.track" context=this sourcelabel="subtitlessourcelabel"' +
                            ' addcomponentlabel="addsubtitlestrack" parenttab=id trackkind="subtitletrack"}}' +
                    '</div>' +
                    '<div data-track-kind="{{CSS.TRACK_CAPTIONS}}" class="tab-pane"' +
                        ...
                        '{{renderPartial "form_components.track" context=this sourcelabel="captionssourcelabel"' +
                            ' addcomponentlabel="addcaptionstrack" parenttab=id trackkind="captiontrack"}}' +
                    '</div>' +
                    '<div data-track-kind="{{CSS.TRACK_DESCRIPTIONS}}" class="tab-pane"' +
                        ...
                        '{{renderPartial "form_components.track" context=this sourcelabel="descriptionssourcelabel"' +
                            ' addcomponentlabel="adddescriptionstrack" parenttab=id trackkind="descriptiontrack"}}' +
                    '</div>' +
                    '<div data-track-kind="{{CSS.TRACK_CHAPTERS}}" class="tab-pane"' +
                        ...
                        '{{renderPartial "form_components.track" context=this sourcelabel="chapterssourcelabel"' +
                            ' addcomponentlabel="addchapterstrack" parenttab=id trackkind="chaptertrack"}}' +
                    '</div>' +
                    '<div data-track-kind="{{CSS.TRACK_METADATA}}" class="tab-pane"' +
                        ...
                        '{{renderPartial "form_components.track" context=this sourcelabel="metadatasourcelabel"' +
                            ' addcomponentlabel="addmetadatatrack" parenttab=id trackkind="metadatatrack"}}' +
                    '</div>' +
                '</div>',
            TRACK: '' +
                '<div class="mb-1 {{CSS.TRACK}}">' +
                    '{{renderPartial "form_components.source" parenttab=parenttab context=this id=CSS.TRACK_SOURCE ' +
                        ' entersourcelabel=sourcelabel urlinput=trackkind}}' +
                    '<div class="form-group">' +
                        '<label class="w-100" for="{{concat parenttab trackkind}}-lang-input-{{suffix}}">' +
                            '{{get_string "srclang" component}}' +
                        '</label>' +
                        '<select id="{{concat parenttab trackkind}}-lang-input-{{suffix}}" ' +
                              ' class="custom-select {{CSS.TRACK_LANG_INPUT}}">' +
                    ...
                    '</div>' +
                    '<div class="form-group">' +
                        '<label class="w-100" for="{{concat parenttab trackkind}}-track-input-{{suffix}}">' +
                          '{{get_string "label" component}}' +
                        '</label>' +
                        '<input id="{{concat parenttab trackkind}}-track-input-{{suffix}}" ' +
                            ' class="form-control {{CSS.TRACK_LABEL_INPUT}}" type="text"/>' +
                    '</div>' +
                    '<div class="form-check">' +
                        '<input id="{{concat parenttab trackkind}}-default-{{suffix}}" type="checkbox" ' +
                            ' class="form-check-input {{CSS.TRACK_DEFAULT_SELECT}}"/>' +
                        '<label for="{{concat parenttab trackkind}}-default-{{suffix}}" class="form-check-label">' +
...
Y.namespace('M.atto_media').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _displayDialogue: function() {
        ...
        if (!('concat' in Y.Handlebars.helpers)) {
            Y.Handlebars.registerHelper('concat', function() {
              var args = [];
    ...
    _attachEvents: function(content, selection) {
        ...
        content.all(SELECTORS.URL_INPUT).on('blur', function(e) {
            this._getMediaHTML(e.currentTarget.ancestor('.atto_form'));
        }, this);
    ...
    _getNextIndex: function(type) {
        if (this.trackCounters.hasOwnProperty(type)) {
            return ++this.trackCounters[type];
        }
        this.trackCounters[type] = 1;
        return this.trackCounters[type];
    },
    ...
    _addTrackComponent: function(element, callback) {
        var tab = element.ancestor('.tab-pane');
        var parenttab = tab.ancestor('.tab-pane');
        var parenttrackType = this._getMediumTypeFromTabPane(parenttab);
        var trackType = this._getTrackTypeFromTabPane(tab);
        var suffix = this._getNextIndex(trackType);
        var context = this._getContext({
            sourcelabel: trackType + 'sourcelabel',
            addcomponentlabel: 'add' + trackType + 'track',
            suffix: suffix,
            parenttab: parenttrackType,
            trackkind: trackType
        });
    ...
    _addMediaSourceComponent: function(element, callback) {
        ...
        var suffix = this._getNextIndex('mediasource' + mediumType);
        var context = this._getContext({
            multisource: true,
            id: CSS.MEDIA_SOURCE,
     ...
     * @private
     */
    _toggleAriaInvalid: function(node, selectors, predicate) {
        selectors.forEach(function(selector) {
            var element = node.all(selector);
     ...
     * @private
     */
    _toggleVisibility: function(node, selector, predicate) {
    ...
    _getMediaHTML: function(form) {
        ...
        var functionName = '_getMediaHTML' + mediumType[0].toUpperCase() + mediumType.substr(1);
        return this[functionName](tabContent);
    },
    ...
    _getMediaHTMLLink: function(tab) {
        var url = tab.one(SELECTORS.URL_INPUT).get('value');
        var context = {
            url: url,
            name: tab.one(SELECTORS.NAME_INPUT).get('value') || false
        };

        this._toggleAriaInvalid(tab, [SELECTORS.URL_INPUT], !context.url);
        this._toggleVisibility(tab, '.' + CSS.MEDIA_URL_WARNING, !context.url);
        if (!context.url) {
          return '';
        }

        return Y.Handlebars.compile(TEMPLATES.HTML_MEDIA.LINK)(context);
    },
    ...
    _getMediaHTMLVideo: function(tab) {
        var poster = tab.one(SELECTORS.POSTER_SOURCE + ' ' + SELECTORS.URL_INPUT).get('value');
    ...
    _getContextForMediaHTML: function(tab) {
        ...
        var sourceSelector =  SELECTORS.MEDIA_SOURCE + ' ' + SELECTORS.URL_INPUT;
        ...
        var sources = tab.all(sourceSelector).get('value').filter(function(source) {
          return !!source;
        }).map(function(source) {
          return {source: source};
        });

        var valid = sources.length && sources.length >= 1;
        this._toggleAriaInvalid(tab, [sourceSelector], !valid);
        this._toggleVisibility(tab, '.' + CSS.MEDIA_URL_WARNING, !valid);
